...
# 1. %s: 변수를 문자열로 바꿔줄때 사용
# 2. %d: 변수를 정수형으로 바꿔줄때 사용
# 3. %f: 변수를 실수형으로 바꿔줄때 사용

# C언어나 java에서도 사용된다. 그러나 python에서는 권장하지않는다 (그래도 많이사용된다)

# str - formatting
# {}를 사용한다 %-formatting과는 달리, 변수의 타입을 볼라도 출력할수있고, 조금더 발전한 방법이다. 
# 비어있는 {}는 빈칸의 개수와 맞게 순서대로 입력해야한다. 중복되는게 있으면 번호를 메겨서 사용가능. (0부터 카운팅 시작)
ed!my_str3 = "{}이는 {}년 생으로 올해 {}살 입니다. \
                \n{}이의 키는 {}이고, 취미는 {}입니다."
